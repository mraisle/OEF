...
query <- "david boraks"
#date_ranges <- c("20210701:20211231", "20220101:20220731", "20220801:20221231", "20230101:20230423")
...
Liz$Key.Author <- as.character("David Boraks")
Liz$All.Authors <- as.character("")

#remove bad row pulls from google 

clean_Liz <- Liz[!grepl("tags", Liz$url),]
clean_Liz <- clean_Liz[!grepl("people", clean_Liz$url),]
clean_Liz <- na.omit(clean_Liz)
clean_Liz <- clean_Liz[!grepl("Fort Bragg", clean_Liz$title),]
clean_Liz <- clean_Liz[!grepl("mental health", clean_Liz$title),]
clean_Liz <- clean_Liz[!grepl("debt ceiling", clean_Liz$title),]
clean_Liz <- clean_Liz[!grepl("politics/", clean_Liz$url),]
clean_Liz <- clean_Liz[!grepl("race-demographics", clean_Liz$url),]
...
write.csv(clean_Liz, "getarticles/davidboraks_june23.csv")
